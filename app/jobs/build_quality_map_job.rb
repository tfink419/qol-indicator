  ...
  def perform(build_status, job_retry=false)
    ...
    begin
      ...
      if build_status.rebuild? && !job_retry
        GroceryStoreQualityMapPoint.where(["lat BETWEEN ? AND ? AND long BETWEEN ? AND ? AND transit_type BETWEEN ? AND ?", south_west_int[0], 
      ...
      until build_status.reload.segment_statuses.reload.all?(&:atleast_isochrones_complete_state?)
        sleep(5)
      ...
      build_status.update!(state:'quality_map-points', percent:0)
